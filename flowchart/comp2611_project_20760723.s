...
# ******* Your codes start here: ********
	...
	blt $t1, $zero, set_x_zero
	bgt $t1, $t2, set_x_width
	j cb_check_y_boundary

set_x_zero:
	sw $zero, 0($t0) # Store x_loc
	j reverse_speed
...
decrease_lf:
	...
	syscall
	jal get_time
	la $t8, time_player_getting_hurt
	sw $v0, 0($t8)
...
increase_lf:
	...
	lw $t2, 0($t2)
	la $t3, increase_life_value
	lw $t3, 0($t3)
	add $t1, $t1, $t3
	ble $t1, $t2, skip_bound_lf
	add $t1, $zero, $t2
	skip_bound_lf:
...
# ******** Your codes start here: **********
	...
	# Current Platform address
	la $t2, current_platform_address
	sw $a0, 0($t2)
...
# ****** Your codes start here **********
	addi $sp, $sp, -4
	sw $ra, 0($sp)
	la $t0, platforms       # platforms
	la $t1, platform_num	
	lw $t1, 0($t1)			# platform_num
platform_loop:
	lw $t2, 4($t0)          # platform_loc_x_left
	lw $t3, 8($t0)			# platform_loc_y_top
	la $t4, player_locs
	la $t5, platform_size
	la $t6, player_size

	lw $t7, 4($t4)          # t7=player_top_y
	blt $t3, $t7, continue_plat_loop

	lw $t8, 4($t6)
	add $t7, $t7, $t8       # t7=player_bottom_y
	bgt $t3, $t7, cp_n_exist

	lw $t7, 0($t4) 	 		# t7=player_loc_x_left
	lw $t8, 0($t5)
	add $t8, $t8, $t2       # t8=platform_loc_y_right
	bgt $t7, $t8, cp_n_exist

	lw $t8, 0($t6)
	add $t7, $t7, $t8
	blt $t7, $t2, cp_n_exist
...
cp_exist:
	...
	la $t7, player_speed
	sw $zero, 0($t7)
	la $t8, platform_y_speed
	lw $t8, 4($t8)
	sw $t8, 4($t7)


	la $t7, current_platform_address
	lw $t8, 0($t7)
	lw $t8, 0($t8)

	lw $t1, 0($t0)
	beq $t1, $t8, old_platform
...
# ******* Your codes start here *******
	...
	lw $a0, 8($t0)
	la $t4, platform_height_distance
	lw $t4, 0($t4)
	add $t4, $a0, $t4
